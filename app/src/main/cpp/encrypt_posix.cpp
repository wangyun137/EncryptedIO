...
#include "jniutils/scoped_local_ref.h"
#include "jniutils/scoped_bytes.h"
#include "jniutils/scoped_utf_chars.h"
...
//init the static field in JniConstants
...
/*
* implements all functions in struct JniConstants
*/
...
static jclass findClass(JNIEnv* env, const char* name) {
    ...
    if (UNLIKELY(result == NULL)) {
        LOGE("EncryptPosix:findClass: find class error: %s", strerror(errno));
        abort();
    }
    return result;
}
...
jobject EncryptPosix::open(JNIEnv* env, jobject, jstring javaPath, jint javaFlags, jint mode) {
    ...
    int fd;
    if (file == NULL) {
        fd = throwIfMinusOne(env, "open", rawOpen(path.c_str(), flags, mode));
    } else {
        throwIfFalse(env, "open", file->open(flags, mode));
        fd = file->getFd();
    }
    addEncryptFile(file);
...
jlong EncryptPosix::mmap(JNIEnv* env, jobject, jlong javaAddress, jlong javaBytesCount,
    ...
    EncryptFile* file = getEncryptFileFromList(fd);
    if (file == NULL) {
        return static_cast<jlong>(rawMmap(suggestedPtr, javaBytesCount, prot, flags, fd, offset));
    }

    return static_cast<jlong>(reinterpret_cast<uintptr_t>(file->mmap(suggestedPtr, javaBytesCount, prot, javaFlags, javaOffset)));
