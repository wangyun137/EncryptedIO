...
void EncryptHead::createHead() {
    ...
    mHead_->size = rawSize(mFile_->getFd());
    mEncrypted_ = true;
}
...
bool EncryptHead::flush() const {
    ...
    if (UNLIKELY(mHead_ == NULL)) {
        LOG("EncryptHead flush error: head is null");
        return true;
    }
...
void EncryptHead::printFileHead() {
    if (UNLIKELY(mHead_ == NULL)) {
        LOG("EncryptHead printFileHead: there is no FileHead");
        return;
    }
    LOGD("*******************************");
    LOGD("EncryptHead:");
...
bool EncryptHead::validateFileHead(const void *headData) {
    ...
    if (tmp->magic == HEAD_MAGIC_V1 && tmp->version == HEAD_VERSION_V1) {
        mHead_ = head;
        return true;
    }
